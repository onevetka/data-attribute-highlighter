...
test ('setErrors action sets error list', () => {
  ...
  const reducer = appErrorReducer(initState, { type: 'setErrors', payload: { errors }})
...
test ('clearErrorsByPath action clear errors only with selected path', () => {
  ...
  const reducer = appErrorReducer(initState, { type: 'clearErrorsByPath', payload: { path: ['emailInput'] } })
